...
function premiseMinicolors() {
	//FJ check if jquery.minicolors loaded
	if (typeof jQuery().minicolors === "function") {
...
function premiseRemoveFile( el ){
	el = jQuery(el) || jQuery('.field .file .premise-btn-remove');
...
/**
 * add post to One Page Nav
 *
 * @return {void}
 */
function premiseAddToOPN() {
	var c = jQuery('#premise_add_post_to_one_page'), s = jQuery('span.ajax-response'), data;
	var premise_post_title     = jQuery( '#premise_post_title' ).val();
	var premise_post_url       = jQuery( '#premise_post_url' ).val();
	var premise_post_object_id = jQuery( '#premise_post_object_id' ).val();
	var premise_post_object    = jQuery( '#premise_post_object' ).val();
	if( c.prop('checked') ){
		data = {
			'action': 'premise_update_this_post',
			'premise_add_post_to_one_page': '1',
			'premise_post_title': premise_post_title,
			'premise_post_url': premise_post_url,
			'premise_post_object_id': premise_post_object_id,
			'premise_post_object': premise_post_object
		};
	}
	else{
		data = {
			'action': 'premise_update_this_post',
			'premise_add_post_to_one_page': '0',
			'premise_post_object_id': premise_post_object_id,
			'premise_post_object': premise_post_object
		};
	}
	s.html( '<i class="fa fa-fw fa-spin fa-spinner"></i>' );
